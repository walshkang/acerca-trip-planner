-- Supabase often installs pgcrypto in the `extensions` schema.
-- Our SECURITY DEFINER RPC pinned search_path to `public`, so `digest()` may not resolve.
-- Fix by including `extensions` in search_path and explicitly qualifying `extensions.digest`.
