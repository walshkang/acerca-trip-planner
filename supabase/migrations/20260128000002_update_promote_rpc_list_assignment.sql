-- Extend promote_place_candidate() to optionally assign to list_items.

CREATE OR REPLACE FUNCTION public.promote_place_candidate(
  p_candidate_id uuid,
...
BEGIN
  ...
  -- Resolve list assignment (use provided list or default list)
  ...
  IF c.status = 'promoted' AND c.promoted_at IS NOT NULL THEN
    ...
    IF v_place_id IS NOT NULL THEN
      IF v_list_id IS NOT NULL THEN
        INSERT INTO public.list_items (list_id, place_id)
        VALUES (v_list_id, v_place_id)
  ...
  IF c.source_id IS NOT NULL THEN
    -- Primary: infer partial unique index on (user_id, source, source_id) WHERE source_id IS NOT NULL
